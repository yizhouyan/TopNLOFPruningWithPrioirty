...
import lof.pruning.LargeCellBasedKnnFindWithPriority.Counters;
...
public class LargeCellBasedKnnFindWithPriority {
	/**
	 * default Map class.
	 *
	 * @author Yizhou Yan
	...
	public static class CellBasedKNNFinderMapper extends Mapper<LongWritable, Text, MetricKey, Text> {
		/**
		...
		protected void setup(Context context) throws IOException, InterruptedException {
			...
			partition_priority = new double[di_numBuckets[0] * di_numBuckets[1] + 1];
					...
					for (int i = 0; i < stats.length; ++i) {
						if (!stats[i].isDirectory() && stats[i].getPath().toString().contains("pp")) {
							...
							while ((line = currentReader.readLine()) != null) {
								...
								partition_priority[tempid] = Double.parseDouble(splitsStr[num_dims * 2 + 1]);
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			context.write(new MetricKey(blk_id, partition_priority[blk_id]), value);
		}
	}

	public static class PriorityPartitioner extends Partitioner<MetricKey, Text> implements Configurable {
	 ...
	 *
	 */
	public static class CellBasedKNNFinderReducer extends Reducer<MetricKey, Text, LongWritable, Text> {
		/**
		 * The dimension of data (set by user, now only support dimension of 2,
		 * if change to 3 or more, has to change some codes)
		 */
		 ...
		 */
		public void reduce(MetricKey key, Iterable<Text> values, Context context)
	...
	public static void main(String[] args) {
		LargeCellBasedKnnFindWithPriority findKnnAndSupporting = new LargeCellBasedKnnFindWithPriority();
		try {
