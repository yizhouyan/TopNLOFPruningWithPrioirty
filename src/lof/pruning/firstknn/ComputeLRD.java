...
public class ComputeLRD {
	...
	public static boolean CalLRDForSingleObject(MetricObject o_S, HashMap<Long, MetricObject> TrueKnnPoints,
			...
			ArrayList<LargeCellStore> leaveNodes, int K) throws IOException, InterruptedException {
		float lrd_core = 0.0f;
	 ...
	 * @throws InterruptedException
	 */
	public static boolean ReCalLRDForSpecial(Context context, MetricObject o_S, HashMap<Long, MetricObject> TrueKnnPoints,
			HashMap<Long, MetricObject> needCalculatePruned, HashMap<Long, MetricObject> lrdHM,
			HashMap<Long, MetricObject> needCalLOF, HashMap<Long, MetricObject> needCalculateLRDPruned, float threshold, int K)
