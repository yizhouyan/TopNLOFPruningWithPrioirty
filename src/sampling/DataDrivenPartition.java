...
//			System.out.println("Chi-Square :" + chisquare + ", Num Of Points: " + pp.getNumData() + ", Priority: " + priority);
			return priority;
		}
						...
						if(needPartition.getNumData()<context.getConfiguration().getInt(SQConfig.strK,3)){  /////////////////////////////////
							if(i== num_dims-1){
								...
								if(outputStr.length()>3){
									...
									chisquarePerPartition[indexForPartitionStore] = computeChiSquareForEachPartition(needPartition);
							...
							for(int k = 0; k< PPs.length; k++){
								if(i == num_dims-1){
									...
									if(outputStr.length()>3){
										...
										// compute chi-square for the partition
										chisquarePerPartition[indexForPartitionStore] = computeChiSquareForEachPartition(PPs[k]);
										indexForPartitionStore++;
									}
								}
	        ...
	        for(int i = 0;i<indexForPartitionStore ;i++){
	        	try {
					mos.write(NullWritable.get(),new Text(i+","+partition_store[i][0]+","+partition_store[i][1]
							 +","+partition_store[i][2]+","+partition_store[i][3] + "," + String.format("%.5f", chisquarePerPartition[i])), outputPPPath+"/pp");
...
//						mos.write(new IntWritable(2),new Text(cell_store[i][j].printCellStore()), outputCellsPath+"/cells");
//					} catch (InterruptedException e) {
//						// TODO Auto-generated catch block
//						e.printStackTrace();
//					}
...
//		MultipleOutputs.addNamedOutput(job, "cells", TextOutputFormat.class, IntWritable.class, Text.class);
